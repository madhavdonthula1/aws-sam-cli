...
class BuildContext:
    ...
    def _identify_failed_resource(self, error_message: str) -> Optional[str]:
        """
        Attempt to identify which function/layer caused the build failure
        by matching the error message against known resource codeuris.

        Parameters
        ----------
        error_message: str
            The error message from the build failure

        Returns
        -------
        Optional[str]
            The logical ID of the failed resource, or None if it cannot be determined
        """
        try:
            for f in self.get_resources_to_build().functions:
                if f.codeuri and f.codeuri in error_message:
                    return f.full_path
            for layer in self.get_resources_to_build().layers:
                if layer.codeuri and layer.codeuri in error_message:
